...
export default function AdminCustomerParcels() {
    ...
    // Filter explicitly bounded to the URL parameter Customer Name!
    const CUSTOMER_ORDERS = MOCK_ORDERS.filter(o => o.customer === decodeURIComponent(customerName));

    const todayStr = today.toISOString().split('T')[0];

    // Filter logic FIRST so stats can dynamically compute based on what's caught in the filter!
    let baseFilteredOrders = CUSTOMER_ORDERS.filter(o => {
    ...
    const backPath = location.state?.from || '/admin/customers';

    return (
        <Layout role="admin" title={`Customer: ${decodeURIComponent(customerName)}'s Drops`} subtitle="Deep dive isolation into this client's routing history">
                            ...
                            {filteredOrders.length === 0 ? (
                                <tr>
                                    <td colSpan={6} style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>
                                        No payload drops logged for {decodeURIComponent(customerName)} during this timeline.
                                    </td>
                                </tr>
                            ) : (
