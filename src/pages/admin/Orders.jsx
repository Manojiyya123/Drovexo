...
import { Search, ChevronDown, ChevronUp, Calendar as CalendarIcon, User, Navigation, Bike, MapPin } from 'lucide-react';
...
export default function AdminOrders() {
    ...
    // Compute unique riders dynamically array
    const uniqueRiders = [...new Set(MOCK_ORDERS.filter(o => o.rider !== '—').map(o => o.rider))];

    // Derived properties for logic
    const todayStr = today.toISOString().split('T')[0];

    // Filter logic FIRST so stats can dynamically compute based on what's caught in the filter!
    let baseFilteredOrders = MOCK_ORDERS.filter(o => {
    ...
    return (
        <Layout role="admin" title="Orders Database" subtitle="Complete order management and filtering">
            ...
            <div className="card" style={{ padding: '20px', marginBottom: '24px', display: 'flex', flexWrap: 'wrap', gap: '20px', alignItems: 'flex-start' }}>
                ...
                <div className="form-group" style={{ flex: 1, minWidth: '200px', margin: 0 }}>
                    <label className="form-label" style={{ fontSize: '11px', textTransform: 'uppercase', color: 'var(--text-muted)' }}>Jump to Rider Drops</label>
                    <div className="select-wrap" style={{ display: 'flex', alignItems: 'center' }}>
                        <Search size={16} style={{ position: 'absolute', left: 12, color: 'var(--text-muted)', zIndex: 2 }} />
                        <input
                            type="text"
                            ...
                            onChange={e => {
                                ...
                                const match = uniqueRiders.find((r, idx) => `RID-00${idx + 1} - ${r}` === val);
                                if (match) navigate(`/admin/rider-drops/${encodeURIComponent(match)}`, { state: { from: '/admin/orders' } });
                            }}
                        />
                        <datalist id="rider-suggestions">
                            {uniqueRiders.map((r, idx) => (
                                <option key={r} value={`RID-00${idx + 1} - ${r}`} />
                            ))}
                        </datalist>
                    </div>
                </div>
                            ...
                            {filteredOrders.length === 0 ? (
                                <tr>
                                    <td colSpan={5} style={{ textAlign: 'center', padding: '40px', color: 'var(--text-muted)' }}>
                                        No drops found matching the current filters.
                                    </td>
                                </tr>
                            ) : (
